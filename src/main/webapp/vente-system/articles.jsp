...
//Mayqdarshi ydkhol l app bla login
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/vente-system/css/style.css">
...
<body>
            ...
            <ul>
                ...
                <li>
                    <a href="/Vente_management/vente-system/cart.jsp">Cart
                        <span style="color:white;background-color: red; border-radius: 5px; position: relative; top: -5px; font-size: 0.7em; padding: 2px 2.5px;">
                            <%=number%>
                        </span>
                    </a>
                </li>
    ...
    <section class="products">
        <h2 style="font-size: 140%; text-shadow:5px 5px 5px red;">Find a frame that's true to you.</h2>
        <div class="all-products">
            <%
            ArticlesDAO dao = (ArticlesDAO) context.getBean("ArticlesDAO");
            List<Articles> articles = dao.getAll();
            %>
            <%
            ...
            <div class="product" style="height:500px; width:300px">
                ...
                <img src="${pageContext.request.contextPath}/vente-system/images/<%=article.getImage()%>">
                
                <div class="product-info">
                    <h5 class="product-title"><%=article.getNom()%></h5>
                    <p class="product-price"><%=article.getPrice()%> $ </p>
                    <p class="product-price"><%=article.getDescription()%></p>
                    <p class="product-price" style="color:grey; font-size: 50%">Quantity left: <%=article.getQuantity()%></p>
                    <a  class="addToCartButton" href="toCart.action?id=<%=article.getId()%>">Add to cart</a>
                </div>
            </div>
            <%
            }
